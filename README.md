...
### 이름 입력 및 검증
- [ ] 쉼표("," )로 구분된 이름 문자열을 입력받는다.
- [ ] 이름 목록(`Names`)을 생성하고, 내부에서 검증을 수행한다.

### 경주 횟수 입력 및 검증
- [ ] 숫자 문자열로 시도 횟수를 입력받는다.
- [ ] 경주 횟수(`Round`)를 생성하고, 내부에서 유효성 검증을 수행한다.

### 게임 진행
- [ ] 입력받은 `Names`로부터 `Cars`를 생성한다.
- [ ] 이동 전략(`MoveStrategy`)과 난수 생성기(`NumberGenerator`)를 준비한다.
- [ ] 라운드 수(`Round`)만큼 다음을 반복한다:
    - [ ] 각 자동차에 대해 전략 평가 후 전진 여부를 결정한다.
    - [ ] 현재 라운드 상태를 스냅샷(`RoundDto`)으로 기록한다.
- [ ] 최종 우승자(복수 가능)를 판정하고 `ResultDto`로 변환한다.

### 결과 출력
- [ ] 차수별 실행 결과를 출력한다.
- [ ] 최종 우승자를 쉼표(",")로 구분해 출력한다.
- [ ] 잘못된 입력 등 예외 발생 시 에러 메시지를 출력한다.
